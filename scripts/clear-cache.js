...
// 配置项：请修改为你的实际 URL
// ============================================
const API_URL = 'https://your-app.onrender.com'; // 修改这里！例如：https://aipush.onrender.com
...
async function clearCache() {
  try {
    ...
    if (!healthCheck.ok) {
      throw new Error(`API 不可访问: ${healthCheck.status} ${healthCheck.statusText}`);
    }

    const stats = await healthCheck.json();
    log(`   ✅ API 连接成功`, 'green');
    log(`   📊 当前缓存状态:`, 'blue');
    log(`      - Redis 可用: ${stats.data.redis.available}`, 'blue');
    log(`      - 内存缓存条目: ${stats.data.memory.size}`, 'blue');
    ...
    if (!clearResponse.ok) {
      throw new Error(`清除失败: ${clearResponse.status} ${clearResponse.statusText}`);
    }
    ...
    const generateResponse = await fetch(`${API_URL}/api/v1/news/ai/generate?count=8`);

    if (!generateResponse.ok) {
      throw new Error(`生成失败: ${generateResponse.status} ${generateResponse.statusText}`);
    }
    ...
    if (generateResult.success && generateResult.data.length > 0) {
      ...
      if (hasTitleCn && hasSummaryCn) {
        ...
        log(`      titleCn: ${hasTitleCn ? '✅' : '❌'}`, hasTitleCn ? 'green' : 'red');
        log(`      summaryCn: ${hasSummaryCn ? '✅' : '❌'}`, hasSummaryCn ? 'green' : 'red');
        log(`\n   可能原因：`, 'yellow');
    ...
    log(`   📊 最新缓存状态:`, 'blue');
    log(`      - Redis 可用: ${finalData.data.redis.available}`, 'blue');
    log(`      - 内存缓存条目: ${finalData.data.memory.size}`, 'blue');
    ...
    if (API_URL === 'https://your-app.onrender.com') {
      log('💡 提示：请先修改脚本中的 API_URL 为你的实际 Render 应用地址！\n', 'yellow');
    }

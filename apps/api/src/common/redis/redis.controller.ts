...
export class RedisController {
  ...
  @ApiResponse({
    status: 200,
    description:
      "Return Redis statistics including memory, keys, and performance metrics",
  })
  async getCacheStats() {
  ...
  async getKeys(
    ...
    const keys = await this.redisService.getKeysWithTTL(
      pattern,
      parseInt(limit),
    );
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: "Clear ALL cache (DANGEROUS - use with extreme caution)",
  })
  @ApiResponse({
    status: 200,
  ...
  async getCacheReport() {
    ...
    const totalKeys = Object.values(keyspace).reduce(
      (sum, count) => sum + count,
      0,
    );
